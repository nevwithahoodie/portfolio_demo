...
function initPlaces() {
  ...
  PLACES.forEach(function(pl, i) {
    ...
    info.innerHTML = '<div class="pcard-name">' + pl.label.toUpperCase() + '</div>'
      + '<div class="pcard-sub">' + pl.photos.length + ' photos &middot; ' + pl.sub + '</div>';
    ...
    d.appendChild(info);
    d.appendChild(badge);
    d.onclick = function() { window.location.href = 'places/' + pl.id + '.html'; };
    g.appendChild(d);
  });
