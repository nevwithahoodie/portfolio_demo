...
function runTypewriter() {
  ...
  const lines = [['N','E','V'], ['T','O','O','K'], ['T','H','A','T','.']];
  el.innerHTML = '';
  ...
  function type() {
    ...
    if (ci === 0 && li > 0) el.insertBefore(document.createTextNode('\n'), cursor);
    ...
    ci++;
    if (ci >= lines[li].length) { li++; ci = 0; setTimeout(type, delay * 1.8); }
...
function animatePageTitle(viewId) {
  ...
  const title = view.querySelector('.ph-title, .bea-title, .bf-title, .ele-title, .p911-title, .mp-hero-title, .about-name');
...
══════════════════════════════════ */
let _petalRAF = null, _petalActive = false;
const _petals = [];
...
function startPetals() {
  ...
  const COLORS = [
    'rgba(244,184,184,0.55)', 'rgba(240,160,160,0.45)',
    'rgba(201,162,39,0.25)',  'rgba(250,200,180,0.4)',
  ...
  function draw() {
    if (!_petalActive) { ctx.clearRect(0, 0, canvas.width, canvas.height); return; }
    ...
    _petals.forEach((p, i) => {
      ...
      p.rot    += p.rotSpeed;
      if (p.y > canvas.height + 30) { _petals[i] = null; spawnPetal(false); }
    });
    for (let i = _petals.length - 1; i >= 0; i--) if (_petals[i] === null) _petals.splice(i, 1);
    ...
    _petals.forEach(p => {
      ...
      ctx.beginPath();
      ctx.ellipse(-p.size * 0.2, -p.size * 0.1, p.size * 0.35, p.size * 0.18, p.rot * .5, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    });
